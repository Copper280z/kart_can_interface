/* USER CODE BEGIN Header */
/**
 ******************************************************************************
 * @file           : main.h
 * @brief          : Header for main.c file.
 *                   This file contains the common defines of the application.
 ******************************************************************************
 * @attention
 *
 * Copyright (c) 2025 STMicroelectronics.
 * All rights reserved.
 *
 * This software is licensed under terms that can be found in the LICENSE file
 * in the root directory of this software component.
 * If no LICENSE file comes with this software, it is provided AS-IS.
 *
 ******************************************************************************
 */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32h7xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */
#include "tusb_config.h"

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define blue_led_Pin GPIO_PIN_4
#define blue_led_GPIO_Port GPIOE
#define red_led_Pin GPIO_PIN_5
#define red_led_GPIO_Port GPIOE
#define green_led_Pin GPIO_PIN_6
#define green_led_GPIO_Port GPIOE
#define ADC_UART4_TX_Pin GPIO_PIN_0
#define ADC_UART4_TX_GPIO_Port GPIOA
#define ADC_UART4_RX_Pin GPIO_PIN_1
#define ADC_UART4_RX_GPIO_Port GPIOA
#define OSD_SPI_MISO_Pin GPIO_PIN_6
#define OSD_SPI_MISO_GPIO_Port GPIOA
#define M6_Pin GPIO_PIN_0
#define M6_GPIO_Port GPIOB
#define M5_Pin GPIO_PIN_1
#define M5_GPIO_Port GPIOB
#define M4_Pin GPIO_PIN_9
#define M4_GPIO_Port GPIOE
#define M3_Pin GPIO_PIN_11
#define M3_GPIO_Port GPIOE
#define M2_Pin GPIO_PIN_13
#define M2_GPIO_Port GPIOE
#define M1_Pin GPIO_PIN_14
#define M1_GPIO_Port GPIOE
#define M7_Pin GPIO_PIN_12
#define M7_GPIO_Port GPIOD
#define M8_Pin GPIO_PIN_13
#define M8_GPIO_Port GPIOD
#define UART1_TX_Pin GPIO_PIN_9
#define UART1_TX_GPIO_Port GPIOA
#define bmi270_cs_Pin GPIO_PIN_15
#define bmi270_cs_GPIO_Port GPIOA
#define bmi088_accel_cs_Pin GPIO_PIN_4
#define bmi088_accel_cs_GPIO_Port GPIOD
#define bmi088_gyro_cs_Pin GPIO_PIN_5
#define bmi088_gyro_cs_GPIO_Port GPIOD

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
